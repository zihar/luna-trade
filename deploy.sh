#!/usr/bin/env bash
# Deploy/update bar-replay ke instance forex-alertd (numpang, tanpa Docker).
#
# Build binary statis arm64 → scp ke instance → restart systemd service.
# Hanya update binary + index.html; service unit & /etc/bar-replay.env
...
echo "==> Kirim binary + index.html ke $HOST…"
